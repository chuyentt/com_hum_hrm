...
<script type="text/javascript">
	...
	js('input:hidden.idtype_guid').each(function(){
		var name = js(this).attr('name');
		if(name.indexOf('idtype_guidhidden')){
			js('#jform_idtype_guid option[value="'+js(this).val()+'"]').attr('selected',true);
		}
	});
	js("#jform_idtype_guid").trigger("liszt:updated");
    });
...
<form action="<?php echo JRoute::_('index.php?option=com_humg_hrm&layout=edit&id=' . (int) $this->item->id); ?>" method="post" enctype="multipart/form-data" name="adminForm" id="employee-form" class="form-validate">

    <div class="form-horizontal">
        <?php echo JHtml::_('bootstrap.startTabSet', 'myTab', array('active' => 'general')); ?>

        <?php echo JHtml::_('bootstrap.addTab', 'myTab', 'general', JText::_('COM_HUMG_HRM_TITLE_EMPLOYEE', true)); ?>
        <div class="row-fluid">
			...
			<?php
				foreach((array)$this->item->idtype_guid as $value): 
					if(!is_array($value)):
						echo '<input type="hidden" class="idtype_guid" name="jform[idtype_guidhidden]['.$value.']" value="'.$value.'" />';
					endif;
				endforeach;
			?>			<div class="control-group">
