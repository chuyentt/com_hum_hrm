...
<script type="text/javascript">
    getScript('//ajax.googleapis.com/ajax/libs/jquery/1.9.1/jquery.min.js', function() {
        jQuery(document).ready(function() {
			...
			jQuery('input:hidden.idtype_guid').each(function(){
				var name = jQuery(this).attr('name');
				if(name.indexOf('idtype_guidhidden')){
					jQuery('#jform_idtype_guid option[value="' + jQuery(this).val() + '"]').attr('selected',true);
				}
			});
					jQuery("#jform_idtype_guid").trigger("liszt:updated");
        });
    });
...
<div class="employee-edit front-end-edit">
    ...
    <form id="form-employee" action="<?php echo JRoute::_('index.php?option=com_humg_hrm&task=employee.save'); ?>" method="post" class="form-validate form-horizontal" enctype="multipart/form-data">
	...
	<div class="control-group">
		<div class="control-label"><?php echo $this->form->getLabel('idtype_guid'); ?></div>
		<div class="controls"><?php echo $this->form->getInput('idtype_guid'); ?></div>
	</div>
	<?php foreach((array)$this->item->idtype_guid as $value): ?>
		<?php if(!is_array($value)): ?>
			<input type="hidden" class="idtype_guid" name="jform[idtype_guidhidden][<?php echo $value; ?>]" value="<?php echo $value; ?>" />
		<?php endif; ?>
	<?php endforeach; ?>
	<div class="control-group">
		<div class="control-label"><?php echo $this->form->getLabel('idtype_number'); ?></div>
		<div class="controls"><?php echo $this->form->getInput('idtype_number'); ?></div>
	</div>
        <div class="control-group">
            <div class="controls">
                <button type="submit" class="validate btn btn-primary"><?php echo JText::_('JSUBMIT'); ?></button>
                <a class="btn" href="<?php echo JRoute::_('index.php?option=com_humg_hrm&task=employeeform.cancel'); ?>" title="<?php echo JText::_('JCANCEL'); ?>"><?php echo JText::_('JCANCEL'); ?></a>
            </div>
        </div>
